...
슬래시 커맨드(/인증, /리더보드, /스탯설정, /구단역할, /내정보, /인증해제, /하이브상태, /진행이벤트, /쿠폰목록)는 최초 1회만 등록하면 된다:
...
8. `/진행이벤트`, `/쿠폰목록` — 컴프야v26 공식 커뮤니티(cpbv-community.com2us.com)의 진행 중 이벤트/사용 가능 쿠폰을 조회 (인증 여부와 무관하게 누구나 사용 가능). 이벤트는 매번 실시간 조회하고, 쿠폰은 커뮤니티 사이트 부하를 줄이기 위해 `COUPON_REFRESH_INTERVAL_SECONDS`(기본 600초=10분) 주기로 백그라운드에서 미리 가져와 캐싱해둔 걸 즉시 반환한다. 쿠폰은 상단 고정된 "쿠폰 모아보기" 공지글의 표를 파싱하는 방식이라, 공지 형식이 크게 바뀌면 `backend/app/community/client.py`를 손봐야 할 수 있다.
