...
8. `/진행이벤트`, `/쿠폰목록` — 컴프야v26 공식 커뮤니티(cpbv-community.com2us.com)의 진행 중 이벤트/사용 가능 쿠폰을 실시간으로 조회 (인증 여부와 무관하게 누구나 사용 가능). 쿠폰은 상단 고정된 "쿠폰 모아보기" 공지글의 표를 파싱하는 방식이라, 공지 형식이 크게 바뀌면 `backend/app/community/client.py`를 손봐야 할 수 있다.
